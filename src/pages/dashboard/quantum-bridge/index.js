...
import TriangularCard from "./IntraCard";
...
export default function Triangular() {
  ...
  const getUpdateCapitalAmount = async () => {
    ...
    try {
      const dataToSend = {
        type: "INTRASINGLEEXCHANGE",
        amount: amountCapital.toString(),
      };
      setIsLoading(true);
      // const response = await dataPostHandler("updateCapitalAmount", dataToSend);
  ...
  const getCapitalAmount = async () => {
    try {
      ...
      if (response.data.responseCode == 200) {
        setAmountCapital(response.data?.result?.intraSingleExchange);
        setIsLoading(false);
      } else {
        setIsLoading(false);
      }
    ...
    if (accessToken) {
      try {
        web.onopen = () => {
          const dataToSend = {
            token: accessToken,
            options: "intraSingleExchangeProfitpath",
            uid:
  ...
  useEffect(() => {
    if (auth?.topHeading) {
      auth?.setTopHeading("Quantum Bridge");
    }
  }, []);
  return (
    <ExchangeContainer>
      <CustomHead
        title="QuantumBridge | Bitedge"
          ...
          <Box className="textfieldBox">
            ...
            <FormHelperText error>
              ...
              (amountCapital == Number?.MAX_VALUE || amountCapital === "")
                ...
                ? "The amount should not exceed the limit."
                : ""}
            </FormHelperText>
          </Box>
        ...
        {!isSocLoading &&
          ...
          triangularArbitrageData.map((data, index) => (
            <Grid item lg={4} md={4} sm={6} xs={12} align="center" key={index}>
              <TriangularCard value={data} type="volume" tradeApi="intra" />
            </Grid>
          ))}
        {!isSocLoading &&
          triangularArbitrageData &&
