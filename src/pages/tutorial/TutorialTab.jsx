...
export default function TutorialTab() {
  const [tabView, setTabView] = useState("Tutorials");
  const themeSetting = useContext(SettingsContext);
  return (
    <>
      <FeaturesTabBoxStyle>
        <Box className="tabBox displayCenter" mt={3}>
          <Button
            className={`tabButton ${tabView === "Tutorials" ? "active" : ""}`}
            onClick={() => setTabView("Tutorials")}
          >
            Tutorials
          </Button>
          {/* <Button
            className={`tabButton ${tabView === "Demos" ? "active" : ""}`}
            onClick={() => setTabView("Demos")}
          >
            Demos
          </Button> */}
        </Box>

        {tabView === "Tutorials" && <TutorialCard />}
        {tabView === "Demos" && <Demo />}
      </FeaturesTabBoxStyle>
    </>
  );
